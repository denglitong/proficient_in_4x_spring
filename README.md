...
`Spring 对 Cache 的支持`、`自定义 Cache 注解的使用`、`与企业级缓存的集成`

- 缓存命中率  
  如果要做缓存，就一定要监控这个指标，来看缓存是否工作良好。
- 过期策略  
  缓存满了，从缓存中移除数据的策略，常见的有 LFU/LRU/FIFO。  
  FIFO：先进先出  
  LRU：最久未使用策略，使用时间距离现在最久的那个数据被移除  
  LFU：最近最少使用策略，一定时间段内使用次数最少的那个数据被移除

Spring 提供了一种可在方法级别进行缓存的抽象，通过使用 AOP 对方法进行织入，如果已经为特定方法入参  
执行过该方法，那么不必执行实际方法就可返回被缓存的结果。需要注意的是，Spring Cache 仅提供了一种  
抽象而未提供具体实现。
